 ...
 * 
 * 
 * <This server is for receiving and sending out message/ encoding outbound message and decoding inbound message >
 * 
 * 
 * 
 * 
...
public class MessageServer {
	...
	private void init(File cfg) {
		...
		try {
			...
			if (!verifyConf(conf))
				throw new RuntimeException("verification of server configuration failed");
		} catch (Exception ex) {
			ex.printStackTrace();
		} finally {
			if (br != null) {
	...
	private static class StartCommandCommunication implements Runnable {
		...
		public void run() {
			...
			try {
				...
				// Start the server.
				logger.info("Starting command server<This is the port for Command communiation> (" + conf.getNodeId() + "), listening on port = "
						+ conf.getCommandPort());
	...
	private static class StartWorkCommunication implements Runnable {
		...
		public void run() {
			...
			try {
				...
				// Start the server.
				logger.info("Starting work server <This is for workport communication> (" + state.getConf().getNodeId() + "), listening on port = "
